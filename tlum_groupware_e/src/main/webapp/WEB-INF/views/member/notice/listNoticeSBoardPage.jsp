...
<body>
	...
	<div>
		<h1>공지사항</h1>	
		<button class="btn btn-primary" onclick="javascript:location.href='insert';">NewBoard</button>
		<select name="searchType">
			...
			<option value="tc" ${cri.searchType eq 'tc'?'selected':'' }>
				제목 + 내용</option>
		</select> <input id="keywordInput" name="keyword" type="text" />
		...
		<div>
			<table class="table table-striped table-hover">
				...
				<c:forEach items="${searchNoticeBoard}" var="NoticeBoardVO">
					<tr>
						<td><a
							href="detail?notice_board_bno=${NoticeBoardVO.notice_board_bno}">${NoticeBoardVO.notice_board_bno }</a>
						</td>
						<td><a
							href="detail?notice_board_bno=${NoticeBoardVO.notice_board_bno}">${NoticeBoardVO.notice_board_title }</a>
						</td>
						<td><a
							href="detail?notice_board_bno=${NoticeBoardVO.notice_board_bno}">${NoticeBoardVO.notice_board_content }</a>
						</td>
						<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
								value="${NoticeBoardVO.notice_board_regdate}" /></td>
					</tr>
				</c:forEach>
			</table>
		</div>
	...
	<script>
		$(".link li a").on(
				...
				function(event) {
					...
					jobForm.attr("action", "listNoticeBoardPage").attr(
							"method", "get");
					jobForm.submit();
				});
		$(function() {
