...
<body>
	<div class="box-header">
		<h3 class="box-title">Limit Board Modify</h3>
	</div>
		...
		<div>
			<div>
				<label for="exampleInput1">번호</label> <input type="text"
					name='limit_board_bno' class="form-control" value="${limitBoard.limit_board_bno}"
					readonly="readonly">
			</div>
			<div>
				<label for="exampleInput2">제목</label> <input type="text"
					name='limit_board_title' class="form-control" value="${limitBoard.limit_board_title}">
			</div>
			<%-- <div>
				<label for="exampleInput3">내용</label>
				<textarea class="form-control" name="limit_board_content" rows="3">${limitBoard.limit_board_content}</textarea>
			</div> --%>
			<!-- 스마트 에디터 본문 -->
...
<script>
 ...
 $("form[role='form']").submit(function(event){
   ...
   var that=$(this);
   var str="<input type='hidden' name='limit_board_content' value='"+showHTML()+"' />";
   that.append(str);
   that.get(0).submit();
});
</script>
