	...
	<h3>상세내용</h3>
         <div>
            <form role="form" action="detail" method="POST">
               	번호<input type="text" name=limit_board_bno value="${limitBoard.limit_board_bno}"/>
               	제목<input type="text" name="limit_board_title" value="${limitBoard.limit_board_title}"/>
               	${limitBoard.limit_board_content}<br/>
               작성자아이디<input type="text" name="emp_id" value="${limitBoard.emp_id}"/><br/>
               작성자<input type="text" name="emp_name" value="${limitBoard.emp_name}"/><br/>
            </form>
  ...
  <div class="timeline-body">{{reply_text}} </div>
    <div class="timeline-footer">
     <a class="btn btn-primary btn-xs" 
       data-toggle="modal" data-target="#replyRemove">replyRemove</a>
    </div>
  </div>         
</li>
{{/each}}
...
var printData=function(replyArr,target,templateObject){
   ...
   $( function() {
      ...
      $("#listBtn").on("click", function() {
         formObj.attr("method", "get");
         formObj.attr("action", "listLimitSBoardPage");
         formObj.submit();
      });
   } );
   ...
   var limit_board_bno=${limitBoard.limit_board_bno}
   var template=Handlebars.compile($('#templateAttach').html());
   var replyPage=1;
   
   getPage("<%=request.getContextPath()%>/limitBoardReplies/"+limit_board_bno+"/"+replyPage);
   ...
   function getPage(pageInfo){
    ...
    $('#replyAddBtn').on('click',function(event){
      ...
      $.ajax({
         type:"post",
         url:"<%=request.getContextPath()%>/limitBoardReplies",
         headers:{
         ...
         dataType:'text',
         data:JSON.stringify({
            "limit_board_bno":limit_board_bno,
            "emp_id":emp_id,
...
//       $('.modal-title').text(reply.attr('data-rno'));
      $('.modal-title').html(reply.attr('data-rno'))d;
   ...
   $('#replyDelBtn').on('click',function(event){
      ...
      $.ajax({
         type:'DELETE',
         url:'<%=request.getContextPath()%>/limitBoardReplies/'+reply_no,
         headers:{
            "Content-Type":"application/json",
